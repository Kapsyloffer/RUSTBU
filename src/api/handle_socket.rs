...
use crate::{
    api::{
        game_handling::{check_exists, create_game, fetch_game},
        move_handling::*,
    },
...
pub async fn handler(ws: WebSocketUpgrade, State(state): State<GameHodler>) -> Response {
    ...
    return ws.on_upgrade(|socket| handle_socket(socket, state, tx));
}

pub async fn handle_socket(mut socket: WebSocket, game_hodler: GameHodler, tx: Sender<()>) {
    let mut rx = tx.subscribe();
    tokio::select! {
        _ = rx.recv() => {
            println!("recv message from other person")
        }
        m_res = socket.recv() => {
            while let Some(ref m_res) = m_res {
                ...
                if let Message::Text(text) = msg {
                    ...
                    match packet {
                        ...
                        GamePacket::CreateGame => {
                            create_game(&mut socket, &game_hodler, tx.clone()).await;
                        }
