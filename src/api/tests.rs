...
fn test_move_parser() {
    ...
    binding
        ...
        .lock()
        .expect("nah")
        .insert(url.clone(), Game::new_game());
...
fn test_move_parser_faulty_5() {
    ...
    assert_ne!(
        g.get_board(Color::White, Color::White).unwrap().get_state()[0][3],
        Tile::Empty
    );
    g.get_board(Color::White, Color::White)
        .unwrap()
...
fn replace_hashmap_in_shared_test() {
    ...
    shared
        ...
        .lock()
        .expect("a")
        .insert(url, game_instance.clone());
